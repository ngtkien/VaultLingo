...
<div class="w-full max-w-5xl mx-auto space-y-6 pb-12">
  ...
  <div
    class="flex items-center gap-2 border-b border-[var(--border-main)] pb-3"
  >
    <button
      type="button"
    ...
    >
      ...
      <span
        class="px-1.5 py-0.5 rounded-full text-[9px] uppercase font-mono bg-amber-500/20 text-amber-700 dark:text-amber-300 font-bold"
        >New</span
      >
    </button>
  </div>
  ...
  {:else}
    ...
    <div class="space-y-4">
      <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-3">
        <div>
          <div class="flex items-center gap-2">
            <span
              class="journal-badge text-[var(--accent-primary)] bg-[var(--accent-primary-light)] px-2.5 py-0.5 rounded text-[10px]"
            >
              Lexicon Engine
            </span>
            {#if wordCount}
          ...
          <h1
            class="font-serif text-3xl sm:text-4xl font-bold tracking-tight text-[var(--text-main)] mt-1"
          >
            Smart Dictionary
          </h1>
          <p class="text-sm font-serif italic text-[var(--text-muted)] mt-1">
            Oxford 6-Block Lexical Architecture with high-speed SQLite lookup
            and AI synthesis.
          </p>
        </div>
        ...
        <div
          class="px-3 py-1 rounded-xl bg-[var(--bg-inner)] border border-[var(--border-main)] text-xs text-[var(--text-muted)] font-mono flex items-center gap-1.5 self-start sm:self-auto"
        >
      ...
      <div class="relative">
        <div class="relative flex items-center">
          <Search
            class="w-5 h-5 text-[var(--text-muted)] absolute left-4 pointer-events-none"
          />
          <input
            type="text"
        ...
        {#if showSuggestions && autoSuggestions.length > 0}
          <div
            class="absolute left-0 right-0 top-full mt-2 bg-[var(--bg-card)] border border-[var(--border-main)] rounded-2xl shadow-xl z-50 overflow-hidden divide-y divide-[var(--border-main)]"
          >
            {#each autoSuggestions as item}
              ...
              >
                <div class="flex items-center gap-2">
                  <span
                    class="font-serif font-bold text-sm text-[var(--text-main)] group-hover:text-[var(--accent-primary)]"
                  >
                    {item.word}
                  </span>
                  <span class="text-xs text-[var(--text-muted)] font-mono"
                  ...
                  >
                  <span
                    class="text-[10px] px-1.5 py-0.5 rounded bg-[var(--bg-inner)] text-[var(--text-subtle)] font-medium"
                  >
                    {item.pos}
                  </span>
                </div>
                <span
                  class="text-xs text-[var(--text-muted)] truncate max-w-xs font-serif italic"
                >
      ...
      {#if recentSearches.length > 0}
        <div
          class="flex items-center gap-2 overflow-x-auto pb-1 text-xs text-[var(--text-muted)] no-scrollbar"
        >
    ...
    {#if currentResult}
      <article
        class="journal-card p-6 sm:p-8 border border-[var(--border-main)] bg-[var(--bg-card)] space-y-6"
      >
        <!-- Word Header: Title, Phonetic, POS, Audio, Actions -->
        <div
          class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-[var(--border-main)]"
        >
          <div>
            <div class="flex items-baseline gap-3 flex-wrap">
              <h1
                class="font-serif text-3xl sm:text-4xl font-bold tracking-tight text-[var(--text-main)]"
              >
              ...
              <span
                class="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-[var(--accent-primary-light)] text-[var(--accent-primary)] border border-[var(--accent-primary-border)]"
              >
              ...
              <!-- Source Badge -->
              <span
                class="px-2 py-0.5 rounded text-[10px] font-mono uppercase font-bold tracking-wider bg-[var(--bg-inner)] text-[var(--text-subtle)] border border-[var(--border-main)]"
              >
                {currentResult.source === "lexicon"
                  ? "SQLite Fast Cache"
                  : currentResult.source.toUpperCase()}
              </span>
            </div>
          ...
          <div class="flex items-center gap-2 shrink-0 flex-wrap">
            ...
            <div
              class="inline-flex items-center rounded-xl border border-[var(--border-main)] bg-[var(--bg-inner)] overflow-hidden divide-x divide-[var(--border-main)] shadow-sm"
            >
              <button
            ...
            >
              {#if isEnriching}
                <RefreshCw
                  class="w-3.5 h-3.5 animate-spin text-[var(--accent-primary)]"
                />
                <span>Enriching...</span>
              {:else}
                <Sparkles class="w-3.5 h-3.5 text-[var(--accent-primary)]" />
                <span
                  >{currentResult.source === "ai"
                    ? "AI Re-enrich"
                    : "AI Enrich"}</span
                >
              {/if}
            ...
            >
              <Zap
                class="w-3.5 h-3.5 text-[var(--accent-primary)] fill-[var(--accent-primary)]/20"
              />
              <span>Practice</span>
            </button>
        ...
        <div class="grid md:grid-cols-2 gap-4">
          <!-- English Definition -->
          <div
            class="p-4.5 rounded-xl bg-[var(--bg-inner)] border border-[var(--border-main)] space-y-2"
          >
            <div class="flex items-center gap-2">
              <span class="w-2 h-2 rounded-full bg-[var(--accent-primary)]"
              ></span>
              <span class="journal-badge text-[var(--text-subtle)] font-bold"
                >English Definition</span
              >
            </div>
            <p
              class="text-sm font-medium text-[var(--text-main)] leading-relaxed"
            >
          ...
          >
            <div class="flex items-center gap-2">
              ...
              <span class="journal-badge text-[var(--text-subtle)] font-bold"
                >Vietnamese Translation</span
              >
            </div>
            <p
              class="text-sm font-serif italic text-[var(--text-main)] leading-relaxed"
            >
              {currentResult.word.definition_vi ||
                "No Vietnamese definition available."}
            </p>
          </div>
        </div>
        ...
        {#if currentResult.word_family && currentResult.word_family.length > 0}
          <div
            class="p-4.5 rounded-xl bg-[var(--bg-inner)] border border-[var(--border-main)] space-y-2.5"
          >
            <div
              class="text-xs font-bold text-[var(--text-subtle)] flex items-center gap-1.5 uppercase tracking-wider"
            >
            ...
            <div class="flex flex-wrap gap-2">
              {#each currentResult.word_family as wf}
                <div
                  class="px-3 py-1.5 rounded-lg bg-[var(--bg-card)] border border-[var(--border-main)] flex items-center gap-2 text-xs"
                >
                  <span
                    class="px-1.5 py-0.5 rounded text-[10px] uppercase font-mono bg-[var(--bg-inner)] text-[var(--text-subtle)]"
                  >
                    {wf.pos}
                  </span>
                  <button
        ...
        {#if currentResult.etymology}
          ...
          >
            ...
            <p
              class="text-xs text-[var(--text-muted)] font-serif italic leading-relaxed"
            >
        ...
        <div
          class="p-4.5 rounded-xl bg-[var(--bg-inner)] border border-[var(--border-main)] space-y-3"
        >
          <div
            class="text-xs font-bold text-[var(--text-subtle)] flex items-center gap-1.5 uppercase tracking-wider"
          >
          ...
          <div class="space-y-2.5">
            {#if currentResult.examples && currentResult.examples.length > 0}
              {#each currentResult.examples as ex}
                <div
                  class="pl-3.5 border-l-2 border-[var(--accent-primary)] space-y-0.5"
                >
                  <p
                    class="text-sm text-[var(--text-main)] font-serif italic leading-relaxed"
                  >
                    “{ex.en}”
                  </p>
                  {#if ex.vi}
            ...
            {:else if currentResult.word.example_en}
              {@const fallbackExamples = parseExamplePairs(
        ...
        {#if (currentResult.synonyms && currentResult.synonyms.length > 0) || (currentResult.antonyms && currentResult.antonyms.length > 0) || (currentResult.collocations && currentResult.collocations.length > 0)}
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {#if currentResult.synonyms && currentResult.synonyms.length > 0}
              <div
                class="p-4 rounded-xl bg-[var(--bg-inner)] border border-[var(--border-main)] space-y-2"
              >
                <span class="journal-badge text-[var(--text-subtle)]"
                  >Synonyms (Click to explore)</span
                >
            ...
            {#if currentResult.collocations && currentResult.collocations.length > 0}
              ...
              >
                <span class="journal-badge text-[var(--text-subtle)]"
                  >High-Yield Collocations</span
                >
                <div class="flex flex-wrap gap-1.5">
                  {#each currentResult.collocations as col}
                    <span
                      class="px-2 py-0.5 text-xs bg-[var(--bg-card)] text-[var(--text-main)] border border-[var(--border-main)] rounded-md font-medium"
                    >
                      {col}
                    </span>
                  {/each}
        ...
        {#if currentResult.nuance_tips || currentResult.mnemonic_hook}
          <div
            class="rounded-xl bg-[var(--bg-inner)] border border-[var(--border-main)] p-4 space-y-3"
          >
            <!-- Tab Header -->
            <div
              class="flex items-center justify-between border-b border-[var(--border-main)] pb-2.5"
            >
              ...
              <span
                class="text-[10px] uppercase font-mono tracking-wider text-[var(--text-subtle)] font-bold hidden sm:inline"
              >
                Linguistic Insights
              </span>
            </div>
            ...
            <div class="pt-1">
              {#if activeInsightTab === "nuance"}
                {#if currentResult.nuance_tips}
                  <div
                    class="text-xs text-[var(--text-main)] leading-relaxed font-sans flex items-start gap-2.5"
                  >
                    <span
                      class="p-1 rounded-md bg-amber-500/10 text-amber-600 dark:text-amber-400 shrink-0"
                    >
                ...
                {:else}
                  <p class="text-xs text-[var(--text-muted)] italic">
                    No specific usage nuance recorded for this word yet. Click
                    "AI Enrich" to synthesize IELTS tips.
                  </p>
                {/if}
              {:else if activeInsightTab === "memory"}
                {#if currentResult.mnemonic_hook}
                  ...
                  >
                    ...
                    <p
                      class="leading-relaxed flex-1 font-medium text-[var(--text-main)]"
                    >
                ...
                {:else}
                  <p class="text-xs text-[var(--text-muted)] italic">
                    No memory hook recorded yet. Click "AI Enrich" to generate a
                    vivid mnemonic association.
                  </p>
                {/if}
              {/if}
            </div>
        ...
        <div class="pt-4 border-t border-[var(--border-main)]">
          ...
          >
            ...
            <span
              >Execution Pipeline Trace ({currentResult.executionTimeMs ||
                4}ms)</span
            >
            {#if showLogs}
          ...
          {#if showLogs && currentResult.debugLogs && currentResult.debugLogs.length > 0}
            <div
              class="mt-3 p-3 rounded-xl bg-[var(--bg-inner)] border border-[var(--border-main)] font-mono text-[11px] text-[var(--text-muted)] space-y-1 overflow-x-auto"
            >
