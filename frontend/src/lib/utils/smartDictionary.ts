...
Required JSON schema:
{
  ...
  const systemPrompt = "You are an expert English lexicographer and etymologist.";
  const responseRaw = await QueryAI(systemPrompt, prompt);
  ...
  const newWord: backend.Word = {
    ...
    level: existing?.word?.level || 'B2',
    topic: existing?.word?.topic || 'ai_enriched',
    topic_title: existing?.word?.topic_title || 'AI Enriched',
    topic_icon: existing?.word?.topic_icon || '✨',
    dict_link: `https://dictionary.cambridge.org/dictionary/english/${encodeURIComponent(term)}`,
    interval: existing?.word?.interval ?? 1,
    repetitions: existing?.word?.repetitions ?? 0,
    ease_factor: existing?.word?.ease_factor ?? 2.5,
